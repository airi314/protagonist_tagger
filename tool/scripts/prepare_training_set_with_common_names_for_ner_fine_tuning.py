...
COMMON_NAMES_FILE = ROOT_DIR.replace(
    "/scripts", "") + "/additional_resources/common_names.txt"
...
# set should be saved
def extract_sentences_for_names_injection(
        titles, number_of_sentences, characters_lists_dir_path, novels_texts_dir_path, training_set_dir):
...
def main(titles_path, characters_lists_dir_path, novels_texts_dir_path,
    ...
    number_sentences_to_extracted = sentences_per_common_name * \
        len(common_names)
    extract_sentences_for_names_injection(
        titles,
