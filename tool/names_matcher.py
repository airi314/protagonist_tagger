...
from tool.diminutives_recognizer import get_names_from_diminutive, create_diminutives_dictionary
...
class NamesMatcher:
    ...
    def recognize_person_entities(self, ner_results, characters):
        ...
        for sent_id, sent_result in enumerate(tqdm(ner_results, leave=False)):
            self.logger.debug("SENTENCE " + str(sent_id) + ": " + sent_result["content"])
            entities = []
            for (ent_start, ent_stop, ent_label, personal_title) in sent_result["entities"]:
            ...
            matcher_results.append({"content":  sent_result["content"], "entities": entities})
    ...
    def find_match_for_person(self, person, personal_title, characters):
        ...
        for index, character in enumerate(characters):
            ...
            ratio_title = fuzz.ratio(((personal_title + " ") if personal_title is not None else "") + person, character)
            partial_ratio = get_partial_ratio_for_all_permutations(person, character)
        ...
        final_match = self.choose_best_match(person, personal_title, potential_matches, characters)
    ...
    def choose_best_match(self, person, personal_title, potential_matches, characters):
        ...
        if self.rules["check_diminutive"] and not final_match:
            potential_names_from_diminutive = get_names_from_diminutive(person.split()[0], self.diminutives_dictionary)
            if potential_names_from_diminutive:
                self.logger.debug("DIMINUTIVE: " + str(potential_names_from_diminutive))
    ...
    def handle_multiple_potential_matches(self, person, personal_title, potential_matches):
        ...
        if self.rules["match_personal_title_100"] and personal_title:
            ...
            for match in potential_matches:
                if match[1] == 100 and personal_title in match[0]:
                    self.logger.debug("FINAL MATCH BY PERSONAL TITLE: " + match[0])
                    return match[0]
        ...
        for match in potential_matches:
            ...
            self.logger.debug("CHARACTER GENDER: " + str(character_gender or "None"))
            ...
            if self.rules["match_personal_title"] and \
                ...
                self.logger.debug("character title different from entity title")
                continue

            if self.rules["match_title_gender"] and \
                    character_gender and title_gender and character_gender != title_gender:
                self.logger.debug("character gender different from title gender")
                continue
            ...
            if self.rules["match_entity_gender"] and \
                ...
                self.logger.debug("character gender different from entity gender")
                continue
